...
curl "https://codeload.github.com/sqlcipher/sqlcipher/zip/refs/tags/v4.5.0" > "$TEMP/source.zip" || exit 1
...
cd "$TEMP/sqlcipher-4.5.0" || exit 1

echo "configuring amalgamation..."
sh configure --enable-tempstore=yes CFLAGS="-DSQLITE_HAS_CODEC" LDFLAGS="/usr/lib/x86_64-linux-gnu/libcrypto.a" > /dev/null || exit 1
